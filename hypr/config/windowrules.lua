...
hl.window_rule({ match = { class = "^(brave-browser)$" }, monitor = MONITOR1, workspace = "1" })
-- Launched via `brave --app=...` (see autostart.lua) so it gets its own window
-- class distinct from "brave-browser", available immediately at window creation.
hl.window_rule({
    match  = { class = ".*[Dd]eezer.*" },
    monitor = MONITOR2,
    float   = true,
    size    = { "monitor_w", "monitor_h*0.5" },
